...
<template>
  <zEditTable :tableColumn="tableColumn" v-model="tableData" :save="saveTable" :items="items" :delete="deleteTable" v-bind="paginationOption" :frontPagination="true"> </zEditTable>
</template>
<script>
export default {
  name: 'zEditTableDemo',
  ...
  methods: {
    ...
    saveTable(callbalk, data) {
      new Promise((resolve) => {
        ...
        const { row } = res;
        data.row.address = Array.isArray(row.address) ? row.address[0] : row.address;
